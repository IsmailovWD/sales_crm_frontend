<template>
  <div>
    <div style="margin: 10px 0">
      <div style="display: flex; gap: 10px; justify-content: flex-end">
        <n-input
          ...
          size="large"
          :placeholder="
            t('search', { name: t('sidebar.delivery-man') })
              .toLocaleLowerCase()
              .charAt(0)
              .toUpperCase() +
            t('search', { name: t('sidebar.delivery-man') })
        ...
        >
          ...
          {{ t("add", { name: t("sidebar.delivery-man") }) }}
        </n-button>
        <n-button
          @click="
            () => {
    ...
    <n-modal
      ...
      :mask-closable="false"
    >
      <Form @closeModal="closeModal" :formValue="formValues" />
    </n-modal>
  </div>
</template>
...
const { t } = useI18n();
const columns = computed<DataTableColumn<DeliveryManAttr>[]>(() => [
  { title: "№", key: "", render: (s, i) => i + 1, width: 80 },
  { title: t("delivery-man.name"), key: "name", resizable: true },
  {
    title: t("delivery-man.salary_type"),
    key: "salary_type",
    resizable: true,
    render: (s) => t(`delivery-man.${s.salary_type}`),
  },
  {
    title: "",
    key: "salary_value",
    resizable: true,
    render: (s) => formatCurrency(s.salary_value),
  },
  {
    title: t("delivery-man.werehouse"),
    key: "werehouse_available",
    resizable: true,
    render: (s) =>
      h(
        NTag,
        {
          size: "small",
          type: s.werehouse_available ? "success" : "error",
          round: true,
        },
        {
          default: () =>
            s.werehouse_available ? t("available") : t("not_available"),
        }
      ),
  },
  {
    title: t("delivery-man.phone"),
    key: "phone_number",
    resizable: true,
    width: 200,
    render: (row) => {
...
const data = ref<DeliveryManAttr[]>([]);
const getData = () => {
  loading.value = true;
