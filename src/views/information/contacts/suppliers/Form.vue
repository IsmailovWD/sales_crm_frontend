<template>
  <n-spin :show="loading">
    <n-form ref="formRef" :model="formValues" :rules="rules">
      <n-form-item path="name" :label="t('suppliers.name')">
        <n-input v-model:value="formValues.name" @keydown.enter.prevent />
      </n-form-item>
      <n-form-item path="phone_number" :label="t('suppliers.phone')">
        <n-input-group>
          <n-select
      ...
      <div style="display: flex; justify-content: flex-end; gap: 10px">
        ...
        <n-button @click="createOrEdit" type="primary">
          {{ "id" in formValue ? t("suppliers.edit") : t("suppliers.add") }}
        </n-button>
      </div>
    </n-form>
  </n-spin>
...
const rules: FormRules = {
  name: {
    ...
    trigger: ["blur", "input"],
    message: () =>
      t("input_errors.{name} is required", { name: t("suppliers.name") }),
  },
  phone_number: {
    type: "string",
    required: true,
