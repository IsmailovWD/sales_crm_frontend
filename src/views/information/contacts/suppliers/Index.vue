<template>
  <div>
    <div style="margin: 10px 0">
      <div style="display: flex; gap: 10px; justify-content: flex-end">
        <n-input
          ...
          size="large"
          :placeholder="
            t('search', { name: t('sidebar.suppliers') })
              .toLocaleLowerCase()
              .charAt(0)
              .toUpperCase() +
            t('search', { name: t('sidebar.suppliers') })
        ...
        >
          ...
          {{ t("add", { name: t("sidebar.suppliers") }) }}
        </n-button>
        <n-button
          @click="
            () => {
    ...
    <n-modal
      ...
      preset="card"
      :style="{ width: '600px' }"
      :title="'id' in formValues ? t('suppliers.edit') : t('suppliers.add')"
...
const columns = computed<DataTableColumn<ContactsAttr>[]>(() => [
  ...
  { title: t("suppliers.name"), key: "name", resizable: true },
  {
